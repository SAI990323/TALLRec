...
def parse_args():
    ...
    parser.add_argument('--seed', type=int, default=1,
                        help='Random seed.')
    parser.add_argument("--gru_layers", type=int, default=4, help = "the layer num for SASRec")
    parser.add_argument("--multi_head", type=int, default=4, help = "the head num for SASRec")
...
class SASRec_with_label(nn.Module):
    ...
    def forward(self, states, state_rate, len_states):
        len_states = np.array(len_states)
...
if __name__ == '__main__':
    result_list_valid = []
    result_list_test = []
    for sample_num in [1,2,4,8,16,32,64,128,256,512]:
        args = parse_args()
        ...
        item_num = 271380 + 1  # total number of items  # total number of items
        ...
        model = SASRec_with_label(args.hidden_factor,item_num, seq_size, args.dropout_rate, device, args.multi_head)
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                model_output = model.forward(seq, seq_rate, len_list).view(-1, item_num)
                ...
                if True:
                    ...
                    if total_step % 5 == 0:
                            _, valid_auc = evaluate_auc_with_history_label(model, 'valid.csv', device)
                            _, test_auc = evaluate_auc_with_history_label(model, 'test.csv', device)
